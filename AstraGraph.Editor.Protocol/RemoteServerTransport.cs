...
/// Uses WSS with bearer token authentication, suitable for remote development scenarios.
/// </summary>
public sealed class RemoteServerTransport : IAuthoringTransport
{
    ...
    public RemoteServerTransport(string? bearerToken = null)
    {
    ...
    public async Task ConnectAsync(Uri endpoint, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            // For remote transport, send an explicit handshake request first
            SessionId = await PerformHandshakeAsync(_cts.Token);
    ...
    private async Task<string?> PerformHandshakeAsync(CancellationToken ct)
    {
        // Send handshake request
        var request = new AuthHandshakeRequestMsg
        {
            ClientVersion = "1.0.0",
            AuthorToken = _bearerToken ?? string.Empty,
            AuthorName = "RemoteClient"
        };
        ...
        if (kind == "auth.handshake.response")
        {
            var resp = JsonSerializer.Deserialize<AuthHandshakeResponseMsg>(msgPayload, AuthoringJsonContext.Default);
            if (resp?.Status != AuthoringStatusCode.Success)
                throw new UnauthorizedAccessException($"Server rejected handshake: {resp?.ErrorMessage}");
